...
class Ssdb
{
    ...
    public function __construct($conf = 'master')
    {
        ...
        $config = $config[$conf] ?? [];
        if (empty($config)) {
            Log::save('error', "SSDB[{$conf}]配置不存在，请在config/database" . EXT . '添加配置！');
            return false;
        }
     ...
     */
    public function getconn(array $config)
    {

        $timeout_ms  = $config['timeout_ms'];

        $timeout_f = (float)$timeout_ms/1000;
        $this->sock = @stream_socket_client("{$config['host']}:{$config['port']}", $errno, $errstr, $timeout_f);
        if(!$this->sock){
